...
if (isset($cms_config_start)) {
	...
	unset($config, $content, $config_gallery_go);
}
...
echo '<h2>Config Child 1:</h2>';

require_once($_SERVER['DOCUMENT_ROOT']."/creator/lib/lead/lead.php");

// Check if isset gallery or/and images
if (isset($_POST['image_maxkb_child1']) && !empty($_POST['image_maxkb_child1']) || isset($_POST['gallery_child1'])) {
	$cms_config_start = 1;
} else {
	if (isset($cms_config_start)) {
		unset($cms_config_start);
...
if (isset($cms_config_start)) {
	...
	if (isset($_POST['image_maxkb_child1']) && !empty($_POST['image_maxkb_child1'])) {
		$config_image = new cms_config_createImageModule($_POST['image_maxkb_child1']);
		$temp_thumb = $config_image->setThumbsConfiguration('image', '_thumbwidth_child1', '_thumbheight_child1');
		$config_image_go = $config_image->getImagesConfiguration($_POST['image_maxkb_child1'], $_POST['image_maxwidth_child1'], $_POST['image_maxheight_child1'], $_POST['image_reqwidth_child1'], $_POST['image_reqheight_child1'], $_POST['cms_config_child1'], $temp_thumb);
		unset($temp_thumb);
	} else {
		$config_image_go = NULL;
	}
	...
	if (isset($_POST['gallery_child1'])) {
		$config_gallery = new cms_config_createGalleryModule();
		if (isset($_POST['gal_thumb_width_child1']) && !empty($_POST['gal_thumb_height_child1'])) {
			$temp_thumb = $config_gallery->setThumbsConfiguration($_POST['gal_thumb_width_child1'], $_POST['gal_thumb_height_child1'], $_POST['cms_config_child1']);
		} else {
			$temp_thumb = NULL;
		}
		$config_gallery_go = $config_gallery->getGalleryConfiguration($_POST['gal_maxkb_child1'], $_POST['gal_max_width_child1'], $_POST['gal_max_height_child1'], $_POST['gal_req_width_child1'], $_POST['gal_req_height_child1'], $_POST['cms_config_child1'], $temp_thumb);
		unset($temp_thumb);
	} else {
		$config_gallery_go = NULL;
	}
	...
	$config = new cms_config_createBasicsModule();
	$config->deleteCommentedByModuleName($_POST['view_name_child1'], $_SERVER['DOCUMENT_ROOT'].'/config/configuration.php');
	$content = 
		$config->startComment($_POST['view_name_child1']) .
		$config_image_go . $config_gallery_go .
		$config->endComment($_POST['view_name_child1']);
	$config->saveInExistedFile($content, $_SERVER['DOCUMENT_ROOT'].'/config/configuration.php');
	$config->deleteEnters($_SERVER['DOCUMENT_ROOT'].'/config/configuration.php');
